...
const TABS = [
  { path: ACCOUNTANT_ROUTES.generateInvoice, label: 'Generate Invoice' },
  { path: ACCOUNTANT_ROUTES.generateReceipt, label: 'Generate Receipt' },
  { path: ACCOUNTANT_ROUTES.bulkInvoiceGeneration, label: 'Bulk Generation' },
  { path: ACCOUNTANT_ROUTES.documentArchive, label: 'Document Archive' },
]

export default function DocumentsPageHeader({ pageTitle, onExport, extraControls }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <Breadcrumb
          items={[
            ...
            { label: 'Invoices & Receipts', to: ACCOUNTANT_ROUTES.generateInvoice },
            { label: pageTitle },
          ]}
        />
      ...
      <nav aria-label="Invoices and receipts module navigation" className="thin-scrollbar -mx-1 flex gap-2 overflow-x-auto px-1 pb-1">
        {TABS.map((tab) => (
          <NavLink
            key={tab.path}
            to={tab.path}
