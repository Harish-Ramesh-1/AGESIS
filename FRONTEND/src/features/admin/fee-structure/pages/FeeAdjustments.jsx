...
import { useFeeAdjustmentsStore } from '../store/feeAdjustmentsStore'
...
export default function FeeAdjustments() {
  const status = useFeeAdjustmentsStore((state) => state.status)
  const error = useFeeAdjustmentsStore((state) => state.error)
  const requests = useFeeAdjustmentsStore((state) => state.requests)
  const fetchRequests = useFeeAdjustmentsStore((state) => state.fetchRequests)
  const actioningId = useFeeAdjustmentsStore((state) => state.actioningId)
  const approve = useFeeAdjustmentsStore((state) => state.approve)
  const reject = useFeeAdjustmentsStore((state) => state.reject)
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeaderSimple title="Fee Adjustments" />

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <SummaryCard icon={Hourglass} label="Pending Admin Approval" value={summary.pendingRequests} status={status} />
      ...
      <div className="relative overflow-hidden rounded-clay border border-white/50 bg-white/30 p-5 shadow-clay backdrop-blur-2xl dark:border-white/10 dark:bg-white/[0.05] sm:p-6">
        ...
        <div className="mb-4 flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold text-slate-900 dark:text-white">Adjustment Requests Awaiting Sign-Off</h2>
        </div>
