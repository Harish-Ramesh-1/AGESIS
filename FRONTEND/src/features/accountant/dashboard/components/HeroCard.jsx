...
export default function HeroCard() {
  const status = useAccountantDashboardStore((state) => state.summaryStatus)
  const hero = useAccountantDashboardStore((state) => state.hero)
  const error = useAccountantDashboardStore((state) => state.summaryError)
      ...
      <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {getGreeting()}, <span className="font-semibold text-slate-900 dark:text-white">{hero.accountantName}</span>
          </p>
          <h2 className="mt-1 text-2xl font-bold tracking-tight text-slate-900 dark:text-white">{hero.department}</h2>
        ...
        <div className="flex shrink-0 flex-col items-start gap-3 rounded-2xl border border-white/40 bg-white/40 px-5 py-4 dark:border-white/10 dark:bg-white/[0.04] sm:items-end">
          <div className="sm:text-right">
            <p className="text-xs text-slate-400 dark:text-slate-500">Today&apos;s Collection</p>
            <p className="mt-0.5 text-2xl font-bold text-slate-900 dark:text-white">
              {formatCurrency(hero.todaysCollection)}
            </p>
            <p className="text-xs text-slate-500 dark:text-slate-400">{hero.todaysCollectionCount} transactions</p>
          </div>
