...
export default function BarDonutChart({ variant, title, description, data, nameKey, valueKey = 'amount', valueFormatter = formatCurrency, trendColor = '#dc2626' }) {
    ...
    <div className="relative h-full overflow-hidden rounded-clay border border-white/50 bg-white/30 p-5 shadow-clay backdrop-blur-2xl dark:border-white/10 dark:bg-white/[0.05] sm:p-6">
      ...
      {isEmpty ? (
        ...
        <Suspense fallback={<Skeleton className="h-56" />}>
          <RevenueChartBase data={data} xKey="label" series={[{ key: valueKey, label: title, color: trendColor }]} valueFormatter={valueFormatter} height={220} />
        </Suspense>
      ) : variant === 'donut' ? (
        <Suspense fallback={<Skeleton className="h-56" />}>
          <PieChart data={data} dataKey={valueKey} nameKey={nameKey} valueFormatter={valueFormatter} height={220} />
        </Suspense>
      ) : (
          ...
          {data.map((row, index) => (
            <div key={row[nameKey]}>
              <div className="mb-1 flex items-center justify-between text-xs">
                ...
                <span className="text-slate-500 dark:text-slate-400">{valueFormatter(row[valueKey])}</span>
              </div>
