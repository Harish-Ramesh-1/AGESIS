...
import { Gauge, PieChart as PieChartIcon, Target, TrendingUp, Trophy } from 'lucide-react'
...
export default function CollectionAnalytics() {
  ...
  const cards = [
    ...
    { icon: PieChartIcon, label: 'Collection Ratio', value: data && data.summary.collectionRatio.toFixed(2) },
  ]
  ...
  return (
    <div className="flex flex-col gap-6">
      <ReportsPageHeader pageTitle="Collection Analytics" onExport={handleExport} />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <OutstandingChart variant="bars" title="Class-wise Collection" data={data?.classWiseCollection} nameKey="className" valueKey="collected" />
        <OutstandingChart variant="bars" title="Department Comparison" data={data?.departmentComparison} nameKey="department" valueKey="collected" />
        <div className="relative overflow-hidden rounded-clay border border-white/50 bg-white/30 p-5 shadow-clay backdrop-blur-2xl dark:border-white/10 dark:bg-white/[0.05] sm:p-6">
          <div aria-hidden="true" className="pointer-events-none absolute inset-x-6 top-0 h-px bg-gradient-to-r from-transparent via-white/70 to-transparent dark:via-white/15" />
