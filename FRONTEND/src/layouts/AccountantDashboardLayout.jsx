...
import { ACCOUNTANT_NAV_SECTIONS, getPageTitle } from '../features/accountant/navigation'
import { useDashboardNotificationStore } from '../features/accountant/dashboard/store/notificationStore'
...
export default function AccountantDashboardLayout() {
  ...
  const notificationItems = useDashboardNotificationStore((state) => state.items)
  const markAllRead = useDashboardNotificationStore((state) => state.markAllRead)
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-slate-100 dark:bg-slate-950">
      ...
      <Sidebar
        ...
        schoolName="AGESIS"
        schoolTagline="Accounts & Finance Portal"
        logoSrc={agesisLogo}
        isCollapsed={isCollapsed}
        onToggleCollapse={() => setIsCollapsed((prev) => !prev)}
        onCollapseSidebar={() => setIsCollapsed(true)}
        ariaLabel="Accountant portal navigation"
      />
      ...
      */}
        ...
        <main className="flex-1 p-4 pb-24 sm:p-6 md:pb-6 lg:p-8">
          <Navbar
            ...
            onSearchSubmit={(query) =>
              navigate(`${ACCOUNTANT_ROUTES.studentDirectory}?q=${encodeURIComponent(query)}`)
            }
          />
          <Outlet />
        </main>
      ...
      <BottomNavBar navSections={ACCOUNTANT_NAV_SECTIONS} />
    </div>
  )
}
