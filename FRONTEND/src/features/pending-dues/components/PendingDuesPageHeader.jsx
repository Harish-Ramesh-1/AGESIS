...
const TABS = [
  { path: ACCOUNTANT_ROUTES.dueList, label: 'Due List' },
  { path: ACCOUNTANT_ROUTES.overdueFees, label: 'Overdue Fees' },
  { path: ACCOUNTANT_ROUTES.reminderManagement, label: 'Reminders' },
  { path: ACCOUNTANT_ROUTES.lateFeeManagement, label: 'Late Fees' },
]

export default function PendingDuesPageHeader({ pageTitle, onExport, extraControls }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <Breadcrumb
          items={[
            ...
            { label: 'Pending Dues', to: ACCOUNTANT_ROUTES.dueList },
            { label: pageTitle },
          ]}
        />
      ...
      <nav aria-label="Pending dues module navigation" className="thin-scrollbar -mx-1 flex gap-2 overflow-x-auto px-1 pb-1">
        {TABS.map((tab) => (
          <NavLink
            key={tab.path}
            to={tab.path}
